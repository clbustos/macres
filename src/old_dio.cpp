...
#include "world.h"
void rawEventByDio(double boundary_freq, double sample_rate, old_fft_complex *xSpec, int xLength, int fft_length, double shiftTime, double f0_floor, double f0_ceil, double *time_axis, int num_frames,
                   double *f0Deviations, double *interpolatedF0,
                   old_fft_plan &forwardFFT, old_fft_plan &inverseFFT, double *equivalentFIR, old_fft_complex *eSpec);
...
void old_dio(double *waveform, int num_samples, int sample_rate, double frame_period,
		 double *time_axis, double *f0)
{
	...
	// Downsampling.
	old_decimateForF0(waveform, num_samples, y, decimation_ratio);
	...
	num_frames = old_GetNumDIOSamples(sample_rate, num_samples, frame_period);
	...
	old_fft_plan			forwardFFT;				// FFT set
	old_fft_complex		*ySpec;	// Spectrogram
	ySpec = (old_fft_complex *)malloc(sizeof(old_fft_complex) * fft_length);
	forwardFFT = old_fft_plan_dft_r2c_1d(fft_length, y, ySpec, FFT_ESTIMATE);
	old_fft_execute(forwardFFT); // Execute FFT.
	...
	old_fft_destroy_plan(forwardFFT);
	double *equivalentFIR;
	equivalentFIR = (double *)malloc(sizeof(double) * fft_length);
	//	old_fft_plan	forwardFFT;				// FFT Set
	old_fft_plan	inverseFFT;
	old_fft_complex		*eSpec;	// Spectrogram
	eSpec = (old_fft_complex *)malloc(sizeof(old_fft_complex) * fft_length);
	forwardFFT = old_fft_plan_dft_r2c_1d(fft_length, equivalentFIR, eSpec, FFT_ESTIMATE);
	inverseFFT = old_fft_plan_dft_c2r_1d(fft_length, eSpec, equivalentFIR, FFT_ESTIMATE);
	...
	*/
	...
	free(f0Deviations);
	old_fft_destroy_plan(forwardFFT);
	old_fft_destroy_plan(inverseFFT);
	free(ySpec);
	for(i = 0;i <= num_bands;i++)
	{
...
void rawEventByDio(double boundary_freq, double sample_rate, old_fft_complex *xSpec, int xLength, int fft_length, double frame_period, double f0_floor, double f0_ceil, double *time_axis, int num_frames,
				   double *f0Deviations, double *interpolatedF0,
				   old_fft_plan &forwardFFT, old_fft_plan &inverseFFT, double *equivalentFIR, old_fft_complex *eSpec)
{
	int i;
	...
	old_fft_execute(forwardFFT); // Apply the FFT
	...
	else
	{
		...
		// Four zero crossings.
		old_interp1(nILocations , nIntervals , nLen , time_axis, num_frames, interpolatedF0Set[0]);
        old_interp1(pILocations , pIntervals , pLen , time_axis, num_frames, interpolatedF0Set[1]);
        old_interp1(dnILocations, dnIntervals, dnLen, time_axis, num_frames, interpolatedF0Set[2]);
        old_interp1(dpILocations, dpIntervals, dpLen, time_axis, num_frames, interpolatedF0Set[3]);
